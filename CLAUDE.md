...
2. **`ex.dispatch(ed, line) -> payload, status` is the shared command core.** The `:` prompt (in `normal.lua`), the control socket (in `editor.lua`), and `.exrc` (future) all call it, so a command means the same thing on every surface (the tmux-like "identical at CLI and in config" property). ex commands are the line-oriented API; `:normal <keys>` is the escape hatch into normal-mode keystrokes.
