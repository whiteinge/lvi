...
describe("normal-mode interpreter", function()
  ...
  describe("insert-mode POSIX commands", function()
    ...
    it("Ctrl-T indents to the next shiftwidth boundary", function()
      local ed = ai("x")
      feed(ed, "i\20" .. ESC)
    ...
    it("Ctrl-D leaves hand-typed indent alone", function()
      local ed = ai("x")
      feed(ed, "i    \4" .. ESC)                     -- POSIX: not autoindent, not its business
      expect(ed.buf:line(1)).to.equal("    x")
    end)
    it("0 Ctrl-D drops the whole autoindent", function()
      local ed = ai("        foo")
      feed(ed, "A\13" .. "0\4")                      -- CR, then 0 ^D
      expect(ed.buf:line(2)).to.equal("")
      expect(ed.cx).to.equal(1)
    end)
    it("^ Ctrl-D drops it here but the next line keeps it", function()
      local ed = ai("        foo")
      feed(ed, "A\13\94\4")                          -- CR, then ^ ^D
      expect(ed.buf:line(2)).to.equal("")
      feed(ed, "x\13")
      expect(ed.buf:line(3)).to.equal("        ")    -- re-derived from the original line
    end)
