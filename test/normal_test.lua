...
describe("normal-mode interpreter", function()
  ...
  -- The POSIX input-mode command set: NUL, ^D, ^H, ^T, ^V (^Q), ^W, <CR>.
  describe("insert-mode POSIX commands", function()
    local function ai(text, over)
      local ed = make(text)
      ed.opts.autoindent, ed.opts.expandtab, ed.opts.shiftwidth = true, true, 4
      for k, v in pairs(over or {}) do ed.opts[k] = v end
      return ed
    end

    it("Ctrl-V inserts the next character literally", function()
      local ed = make("x")
      feed(ed, "i\22\9\22\27" .. ESC)                -- ^V<Tab> ^V<Esc>
      expect(ed.buf:line(1)).to.equal("\9\27x")
    end)
    it("Ctrl-Q is a synonym", function()
      local ed = make("x")
      feed(ed, "i\17\9" .. ESC)
      expect(ed.buf:line(1)).to.equal("\9x")
    end)
    it("Ctrl-V keeps a multibyte character whole", function()
      local ed = make("x")
      feed(ed, "i\22\195\169" .. ESC)
      expect(ed.buf:line(1)).to.equal("\195\169x")
    end)
    it("a quoted newline still breaks the line (POSIX exempts it)", function()
      local ed = make("ab")
      feed(ed, "li\22\13" .. ESC)                    -- ^V<CR> at column 2
      expect(ed.buf:nlines()).to.equal(2)
      expect(ed.buf:line(1)).to.equal("a")
      expect(ed.buf:line(2)).to.equal("b")
    end)

    it("Ctrl-T indents by one shiftwidth", function()
      local ed = ai("x")
      feed(ed, "i\20" .. ESC)
      expect(ed.buf:line(1)).to.equal("    x")
    end)
    it("Ctrl-D takes back what Ctrl-T added", function()
      local ed = ai("        foo")
      feed(ed, "A\13")                               -- autoindent carries 8 spaces
      expect(ed.buf:line(2)).to.equal("        ")
      feed(ed, "\20"); expect(ed.buf:line(2)).to.equal("            ")
      feed(ed, "\4");  expect(ed.buf:line(2)).to.equal("        ")
      feed(ed, "\4");  expect(ed.buf:line(2)).to.equal("    ")
      expect(ed.cx).to.equal(5)
    end)
    -- The deviation from POSIX (which scopes ^D to autoindent, and only with the
    -- cursor sitting right after it): both keys are >> and << on the line.
    it("Ctrl-D dedents hand-typed indent too", function()
      local ed = ai("x")
      feed(ed, "i    \4" .. ESC)
      expect(ed.buf:line(1)).to.equal("x")
    end)
    it("both keys work with text typed after the indent", function()
      local ed = ai("        foo")
      feed(ed, "A\13bar")                            -- CR, autoindent, then text
      expect(ed.buf:line(2)).to.equal("        bar")
      feed(ed, "\4")
      expect(ed.buf:line(2)).to.equal("    bar")
      expect(ed.cx).to.equal(8)                      -- cursor stays after "bar"
      feed(ed, "\20")
      expect(ed.buf:line(2)).to.equal("        bar")
      expect(ed.cx).to.equal(12)
    end)
    it("Ctrl-D stops at column 1", function()
      local ed = ai("x")
      feed(ed, "i\4\4" .. ESC)
      expect(ed.buf:line(1)).to.equal("x")
    end)
    it("a tabbed indent stays tabbed", function()
      local ed = ai("x", { expandtab = false, shiftwidth = 8, tabstop = 8 })
      feed(ed, "i\20\20" .. ESC)
      expect(ed.buf:line(1)).to.equal("\9\9x")
      feed(ed, "A\4" .. ESC)
      expect(ed.buf:line(1)).to.equal("\9x")
    end)
    it("Esc still trims a line Ctrl-T only indented", function()
      local ed = ai("    foo")
      feed(ed, "A\13\20" .. ESC)                     -- CR, ^T, nothing typed
      expect(ed.buf:line(2)).to.equal("")
    end)
    it("Esc trims a line holding only its autoindent, but not typed blanks", function()
      local ed = ai("    foo")
      feed(ed, "A\13" .. ESC)
      expect(ed.buf:line(2)).to.equal("")
      local ed2 = ai("x")
      feed(ed2, "i    " .. ESC)                      -- the user typed these
      expect(ed2.buf:line(1)).to.equal("    x")
    end)

    it("NUL replays the last insert and leaves insert mode", function()
      local ed = make("z")
      feed(ed, "ifoo" .. ESC)
      expect(ed.last_insert).to.equal("foo")
      feed(ed, "A\0")
      expect(ed.buf:line(1)).to.equal("foozfoo")
      expect(ed.mode).to.equal("normal")
    end)
    it("NUL replays a multi-line insert", function()
      local ed = make("z")
      feed(ed, "ia\13b" .. ESC)
      expect(ed.last_insert).to.equal("a\nb")
      feed(ed, "o\0")
      expect(ed.buf:line(3)).to.equal("a")
      expect(ed.buf:line(4)).to.equal("b")
    end)
    it("NUL is only special as the first character of an insert", function()
      local ed = make("z")
      feed(ed, "ifoo" .. ESC)
      feed(ed, "iab\0c" .. ESC)                      -- mid-insert: ignored, not a replay
      expect(ed.buf:line(1):find("\0", 1, true)).to.be(nil)   -- and never inserted raw
      expect(ed.buf:line(1):find("abc", 1, true)).to_not.be(nil)
      expect(ed.buf:line(1):find("foofoo", 1, true)).to.be(nil)
    end)
  end)
