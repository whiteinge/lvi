...
describe("command-backed registers", function()
  ...
  describe("the unnamed register as a capture seam", function()
    ...
    it("backs the unnamed register via the rc-safe \"\" spelling", function()
      local ed = make("hello\nworld")
      expect(select(2, ex.dispatch(ed, 'register "" write ring'))).to.equal("ok")
      expect(ed.reg_backends['"'].write).to.equal("ring")   -- "" aliased to "
      feed(ed, "yy")
      expect(#ed.writes).to.equal(1)
      expect(ed.writes[1].cmd).to.equal("ring")
    end)
